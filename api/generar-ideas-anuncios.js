// Endpoint server-side para el Jefe de Anuncios y Campañas de Paga -- genera 9 ideas LIGERAS
// (titulo+hook+porque, sin guion completo) a partir del mismo CONTEXTO DEL NEGOCIO que usa Jefe
// Evergreen + sus Notas de Comunicación Evergreen + el Radar de Mercado. Multi-tenant, sin datos
// hardcoded de ninguna marca. El detalle completo se genera aparte, solo para lo que se apruebe
// (ver api/generar-detalle-anuncio.js) para no gastar tokens en ideas que no se van a usar.
...
const PROMPT_PATH = path.join(__dirname, '..', 'prompts', 'system-prompt-ideas-anuncios.md');
...
function formatearProducto(items, productoId) {
  ...
  if (p.tipo) partes.push(p.tipo);
  if (p.notas) partes.push(`notas: ${p.notas}`);
  return 'PRODUCTO ESPECÍFICO A ENFOCAR (las 9 ideas deben girar en torno a este producto puntual, no al grupo completo):\n- ' + partes.join(' · ');
}
  ...
  try {
    ...
    const anthropicRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 2500,
        system,
        messages: [{ role: 'user', content: 'Genera las 9 ideas de anuncios pedidas, en el formato JSON indicado. Sé breve por campo -- son ideas de referencia para elegir, no el anuncio terminado.' }],
      }),
    });
