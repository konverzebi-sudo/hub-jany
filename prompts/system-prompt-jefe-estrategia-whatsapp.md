...
- s1_bienvenida NUNCA es un párrafo de presentación tipo folleto o ficha técnica ("Somos [negocio], llevamos X años, ofrecemos Y" y punto). Eso se siente robotizado y sin energía, y mata la conversación en el primer mensaje. Tiene que sonar cálido, con ganas reales de ayudar, y SIEMPRE terminar en una pregunta concreta y específica que invite a seguir platicando -- una pregunta ligada a QUÉ puede comprar o qué necesita el cliente de entre las opciones reales del negocio (nunca una pregunta genérica tipo "¿en qué te puedo ayudar?" ni quedarse sin pregunta). Antes de responder, revisa el s1_bienvenida que vayas a entregar: si termina en punto en vez de en pregunta, o si es solo presentación del negocio, todavía está mal -- corrígelo, sin importar si el CONTENIDO ACTUAL DE LAS TARJETAS ya traía ese mismo error (no lo repitas solo por "conservar lo que ya había"). Ejemplo de la diferencia: MAL -- "Somos el gimnasio X, 10 años de experiencia, ofrecemos pesas y clases grupales." BIEN -- "¿Buscas algo para ponerte en forma tú solo o prefieres el ambiente de clases grupales? Así te cuento justo lo que te conviene 💪".
